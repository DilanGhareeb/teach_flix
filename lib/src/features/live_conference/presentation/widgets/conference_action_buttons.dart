...
class ConferenceActionButtons extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (!conference.canJoin) {
      return JoinTimeWarning(conference: conference);
    }
    ...
    if (remainingMinutes > 0) {
      return Column(
        children: [
          JoinTimeWarning(conference: conference),
  ...
  Widget _buildStudentButton(BuildContext context, AppLocalizations l10n) {
    ...
    final hasPurchased = conference.enrolledStudentIds.contains(userId);

    if (hasPurchased || conference.price == 0) {
      ...
      child: ElevatedButton.icon(
        onPressed: () {
          context.read<LiveConferenceBloc>().add(
            JoinConferenceRequested(conference.id),
          );
    ...
    showDialog(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text('${l10n.conferenceTitle}: ${conference.title}'),
            const SizedBox(height: 8),
            Text(
              '${l10n.conferencePrice}: ${Formatter.formatIqd(conference.price)}',
            ),
            const SizedBox(height: 8),
            Text('Your Balance: ${Formatter.formatIqd(userBalance)}'),
            const SizedBox(height: 16),
            if (userBalance < conference.price)
              Container(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: userBalance >= conference.price
                ? () {
                    Navigator.pop(dialogContext);
                    context.read<LiveConferenceBloc>().add(
                      PurchaseConferenceAccessRequested(conference.id),
                    );
                  }
                : null,
            child: Text(l10n.yes),
